...
class KelasController
{
    ...
    {
        $query = Kela::withCount(['siswa' => fn ($q) => $q->where('kelas_siswa.status', 'aktif')])
    ...
    public function store(Request $request)
    {
        ...
        return $this->success($kelas, 'Kelas berhasil ditambahkan', 201);
    }

    public function show(Request $request, Kela $kela)
    {
        ...
        $kela->load(['siswa.sekolah', 'pertemuans']);
    ...
    public function update(Request $request, Kela $kela)
    {
        ...
        return $this->success($kela, 'Kelas berhasil diperbarui');
    }
    ...
    public function addSiswa(Request $request, Kela $kela)
    {
        ...
        $sudahAktifDiKelasLain = $siswa->kelas()->wherePivot('status', 'aktif')->exists();

        if ($sudahAktifDiKelasLain) {
            return $this->error('Siswa ini sudah aktif di kelas lain. Satu siswa hanya boleh aktif di satu kelas — keluarkan dulu dari kelas sebelumnya.', 422);
        }
